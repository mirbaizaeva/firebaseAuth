...
class RegisterController: UIViewController {
    ...
    private let signUpButton = CustomButton(title: "Sing Up", hasBackground: true, fontSize: .big)
    private let signInButton = CustomButton(title: "Already have an account? Sing In", hasBackground: true, fontSize: .med)
    
    private let termsTextView: UITextView = {
        let attributedString = NSMutableAttributedString(string: "By creating an account, you agree to out Terms & Conditions and you acknowledge that you have read our Privacy Police")
...
let registerUserRequest = RegiserUserRequest(
    ...
    password: self.passwordField.text ?? ""
        ...
        if !Validator.isValidUsername(for: registerUserRequest.username){
            AlertManager.showInvailidUsernameAlert(on: self)
            return
        }
        if !Validator.isValidEmail(for: registerUserRequest.email){
            AlertManager.showInvailidEmailAlert(on: self)
            return
        }
        if !Validator.isPasswordValid(for: registerUserRequest.password){
            AlertManager.showInvailidPasswordAlert(on: self)
            return
        }
